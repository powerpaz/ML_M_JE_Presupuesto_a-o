// app.v10.js — mejora de mapa (invalidateSize, bounds EC), alias lat/lon, parse robusto
...
const CSV_PATH = "rubros_csv.csv"; // fallback CSV (;) ya limpio

// Ecuador approx bounds
const ECUADOR_BOUNDS = L.latLngBounds([[-5.2,-81.6],[1.9,-75.0]]);
...
function parseCoord(v){
  ...
  // If values look like micro/milli degrees (e.g., -7900115439), scale down
  // Bring longitude into [-180,180] and latitude into [-90,90]
  let tries = 0;
  while (Math.abs(num) > 180 && tries < 6) { num /= 10; tries++; }
  return num;
}
  const num = Number(s);
  return Number.isFinite(num)?num:undefined;
...
async function init(){
  map = L.map('map', { zoomControl:true }).fitBounds(ECUADOR_BOUNDS);
  ...
  setTimeout(()=> map.invalidateSize(), 300); // asegura tamaño correcto
}
...
async function loadData(){
  if(hasSupabase()){
    ...
    const { data, error } = await fetchInstituciones({ zona:null, nivel:null, anio:null, limit:20000 });
...
function setupFilters(){
  ...
  rawRows.forEach(row=>{
    ...
    const cant = get(row,['CANTON','CANTÓN','CANTON_','CANT']);
    const zona = get(row,['ZONA','ZONAS']);
...
function rowPasses(row){
  ...
  const cant = normalizeText(get(row,['CANTON','CANTÓN','CANTON_','CANT']));
  const zona = normalizeText(get(row,['ZONA','ZONAS']));
...
function updateTable(){
  ...
  filteredRows.forEach(row=>{
    const ml = parseNum(get(row,['MD_MONTO USD$','MD_MONTO_USD','MD_MONTO_USD$']));
    const mo = parseNum(get(row,['M_MONTO USD$','M_MONTO_USD','M_MONTO_USD$']));
    const je = parseNum(get(row,['JE_MONTO USD$','JE_MONTO_USD','JE_MONTO_USD$']));
...
function updateMap(){
  ...
  filteredRows.forEach(row=>{
    const lat = parseCoord(get(row,['LATITUD','LAT','Y','LATITUDE','LATITUD_WGS84']));
    const lon = parseCoord(get(row,['LONGITUD','LON','LONG','X','LONGITUDE','LONGITUD_WGS84']));
    ...
    const md = parseNum(get(row,['MD_MONTO USD$','MD_MONTO_USD','MD_MONTO_USD$']));
    const m  = parseNum(get(row,['M_MONTO USD$','M_MONTO_USD','M_MONTO_USD$']));
    const je = parseNum(get(row,['JE_MONTO USD$','JE_MONTO_USD','JE_MONTO_USD$']));
  ...
  else map.fitBounds(ECUADOR_BOUNDS);
