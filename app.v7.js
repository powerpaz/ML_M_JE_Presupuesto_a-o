/* v7: incluye provincias_simplificado.geojson + alias y popups pedidos */
...
function regimeColor(r){
  const R = normalizeText(r).toUpperCase();
...
function setupFilters(){
  ...
  rawRows.forEach(row=>{
    const prov = get(row,['PROVINCIA','provincia','provincia_adm','province']);
    const cant = get(row,['CANTON','CANTÓN','canton','canton_adm']);
    const zona = get(row,['ZONA','zona','zona_educativa','zona educativa']);
    const nivel = get(row,['NIVEL DE EDUCACION','NIVEL DE EDUCACIÓN','nivel','nivel_educacion','nivel de educación']);
    const anio = get(row,['AUX_AÑO DE DOTACIÓN','AUX_ANIO DE DOTACION','aux_año_de_dotación','aux_anio_de_dotacion','anio_dotacion','año_dotación','AUX_AÑO DOTACION']);
...
function rowPasses(row){
  ...
  const amie = normalizeText(get(row,['AMIE','amie','codigo_amie','cod_amie']));
  const prov = normalizeText(get(row,['PROVINCIA','provincia','provincia_adm','province']));
  const cant = normalizeText(get(row,['CANTON','CANTÓN','canton','canton_adm']));
  const zona = normalizeText(get(row,['ZONA','zona','zona_educativa','zona educativa']));
  const nivel = normalizeText(get(row,['NIVEL DE EDUCACION','NIVEL DE EDUCACIÓN','nivel','nivel_educacion','nivel de educación']));
  const anio = normalizeText(get(row,['AUX_AÑO DE DOTACIÓN','AUX_ANIO DE DOTACION','aux_año_de_dotación','aux_anio_de_dotacion','anio_dotacion','año_dotación','AUX_AÑO DOTACION']));
...
function updateTable(){
  ...
  filteredRows.forEach(row=>{
    const ml = parseNum(get(row,[
      'material_ludico','material_lúdico','C2_RUBRO 2.3.1','c2_rubro_2_3_1','rubro_2_3_1','C2: Material Lúdico (Didáctico)'
    ]));
    const mo = parseNum(get(row,[
      'mobiliario','C2_RUBRO 2.3.2','c2_rubro_2_3_2','rubro_2_3_2','C2: Mobiliario'
    ]));
    const je = parseNum(get(row,[
      'juegos_exteriores','juegos_exteriores_(equipamiento)','C2_RUBRO 2.3.3','c2_rubro_2_3_3','rubro_2_3_3','C2: Juegos Exteriores (Equipamiento)'
    ]));
...
function updateMap(){
  ...
  const bounds = [];
  filteredRows.forEach(row=>{
    const lat = parseFloat(get(row,['LATITUD','lat','latitude','y']));
    const lon = parseFloat(get(row,['LONGITUD','lon','lng','long','x']));
    if(!Number.isFinite(lat) || !Number.isFinite(lon)) return;

    const amie = get(row,['AMIE','amie','codigo_amie','cod_amie']) || '';
    const institucion = get(row,[
      'INSTITUCION','INSTITUCIÓN','institución','institucion','nombre','ie','NOMBRE DE LA INSTITUCION'
    ]) || '';
    const sosten = get(row,['SOSTENIMIENTO','sostenimiento']) || '';
    const nivel = get(row,[
      'NIVEL DE EDUCACION','NIVEL DE EDUCACIÓN','nivel','nivel_educacion','nivel de educación'
    ]) || '';
    const auxMat = get(row,['Aux_IE_Material','AUX_IE_MATERIAL','aux_ie_material','aux_ie_materiales']) || '';

    const md = parseNum(get(row,[
      'MD_MONTO USD$','MD MONTO USD$','MD_MONTO_USD$','MD_MONTO_USD','MD MONTO','MD_USD','MD_MONTO'
    ]));
    const m  = parseNum(get(row,[
      'M_MONTO USD$','M MONTO USD$','M_MONTO_USD$','M_MONTO_USD','M MONTO','M_USD','M_MONTO'
    ]));
    const je = parseNum(get(row,[
      'JE_MONTO USD$','JE MONTO USD$','JE_MONTO_USD$','JE_MONTO_USD','JE MONTO','JE_USD','JE_MONTO'
    ]));
    const sumaMontos = [md,m,je].reduce((acc,v)=>acc+(Number.isFinite(v)?v:0),0);

    const regimen = get(row,['REGIMEN','RÉGIMEN','regimen','régimen']) || '';
    const zona = get(row,['ZONA','zona','zona_educativa','zona educativa']) || '';
